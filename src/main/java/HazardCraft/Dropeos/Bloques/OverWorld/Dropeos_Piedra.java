...
public class Dropeos_Piedra 
{
	...
	@SubscribeEvent
	public void Piedra (HarvestDropsEvent event)
	{
		if (event.getHarvester() != null)
		{
			if (event.getState().toString().contains("[variant=stone]") ||
				event.getState().toString().contains("[variant=granite]") ||
				event.getState().toString().contains("[variant=diorite]") ||
				event.getState().toString().contains("[variant=andesite]"))
			{
