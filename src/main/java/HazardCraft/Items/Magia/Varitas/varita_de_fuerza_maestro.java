...
public class varita_de_fuerza_maestro extends Registro_Items {

	private static final int durabilidad = 40;
	
	public varita_de_fuerza_maestro(String name) {
	...
	{
		if(player.getHeldItem(hand.MAIN_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_FUERZA_MAESTRO)) 
		{
			funcion_varita(world, player, hand, "main");
		}
		else if(player.getHeldItem(hand.OFF_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_FUERZA_MAESTRO)) 
		{
	...
	public static void funcion_varita(World world, EntityPlayer player, EnumHand hand, String cual_mano) 
	{
		if(cual_mano.equalsIgnoreCase("main")) 
		{
			...
			{
				player.addPotionEffect(new PotionEffect(Potion.getPotionById(5), 24*(30) , 1));
