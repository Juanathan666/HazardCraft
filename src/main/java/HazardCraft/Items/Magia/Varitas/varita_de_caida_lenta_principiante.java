...
public class varita_de_caida_lenta_principiante extends Registro_Items {

	private static final int durabilidad = 12;
	
	public varita_de_caida_lenta_principiante(String name) 
	{
		super(name);
	...
	{
		if(player.getHeldItem(hand.MAIN_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_CAIDA_LENTA_PRINCIPIANTE)) 
		{
			funcion_varita(world, player, hand, "main");
		}
		else if(player.getHeldItem(hand.OFF_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_CAIDA_LENTA_PRINCIPIANTE)) 
		{
	...
	public static void funcion_varita(World world, EntityPlayer player, EnumHand hand, String cual_mano) 
	{
		if(cual_mano.equalsIgnoreCase("main")) 
		{
			...
			if(world.getBlockState(posb).getBlock()==Blocks.AIR) 
			{
				...
				{
					player.addPotionEffect(new PotionEffect(Pociones.CAIDA_LENTA_EFECTO, 24*(4) , 1));
