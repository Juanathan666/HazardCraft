...
public class varita_de_regeneracion_basica extends Registro_Items {

	private static final int durabilidad = 5;
	
	public varita_de_regeneracion_basica(String name) {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
			...
			if(player.getHeldItem(hand.MAIN_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_REGENERACION_BASICA)) {
			
		funcion_varita(world, player, hand, "main");
		}else if(player.getHeldItem(hand.OFF_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_REGENERACION_BASICA)) {
	...
	public static void funcion_varita(World world, EntityPlayer player, EnumHand hand, String cual_mano) {
		...
		if(cual_mano.equalsIgnoreCase("main")) {
			...
			if(!(damage==durabilidad)) {
				player.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("minecraft:regeneration"), 24*(2) , 1));
