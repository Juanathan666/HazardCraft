...
public class varita_de_rayos_basica extends Registro_Items
{
	private static final int durabilidad = 5;

	public varita_de_rayos_basica(String name) 
	{
		super(name);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer player, EnumHand hand) 
	{
		...
		for(int i = (int) lookingAt.getBlockPos().getY(); i>0 && !ya; i--) 
		{
	        ...
	        {
	    		if(player.getHeldItem(hand.MAIN_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_RAYOS_BASICA)) 
	    		{
	    			funcion_varita(worldIn, player, hand, "main",lookingAt.getBlockPos().getX(), i, lookingAt.getBlockPos().getZ());

	    		}
	    		else if(player.getHeldItem(hand.OFF_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_RAYOS_BASICA)) 
	    		{
	...
	public static void funcion_varita(World world, EntityPlayer player, EnumHand hand, String cual_mano, int x, int y, int z) 
	{
		if(cual_mano.equalsIgnoreCase("main")) 
		{
			...
			{
				EntityLightningBolt l = new EntityLightningBolt(world, x, y+1, z, true);
	    		world.spawnEntity(l);	
