...
public class varita_de_explosiones_principiante extends Registro_Items
{
	private static final int durabilidad = 12;

	public varita_de_explosiones_principiante(String name) 
	{
		super(name);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer player, EnumHand hand) 
	{
		...
		for(int i = (int) lookingAt.getBlockPos().getY(); i>0 && !ya; i--) 
		{
			...
			{
				if(player.getHeldItem(hand.MAIN_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_EXPLOSIONES_PRINCIPIANTE)) 
				{
	                funcion_varita(worldIn, player, hand, "main",lookingAt.getBlockPos().getX(), i, lookingAt.getBlockPos().getZ());

	    		}
				else if(player.getHeldItem(hand.OFF_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_EXPLOSIONES_PRINCIPIANTE)) 
				{
	...
	public static void funcion_varita(World world, EntityPlayer player, EnumHand hand, String cual_mano, int x, int y, int z) 
	{
		if(cual_mano.equalsIgnoreCase("main")) 
		{
			...
			{
                world.newExplosion(world.getEntityByID(1), x, y+1, z, 2, false, true);					
