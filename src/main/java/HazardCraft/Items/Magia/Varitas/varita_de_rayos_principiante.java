...
public class varita_de_rayos_principiante extends Registro_Items
{
	private static final int durabilidad = 12;

	public varita_de_rayos_principiante(String name) 
	{
		super(name);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer player, EnumHand hand) 
	{
		...
		for(int i = (int) lookingAt.getBlockPos().getY(); i>0 && !ya; i--) 
		{
	        ...
	        {
	        	if(player.getHeldItem(hand.MAIN_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_RAYOS_PRINCIPIANTE)) 
	        	{   
	        		funcion_varita(worldIn, player, hand, "main",lookingAt.getBlockPos().getX(), i, lookingAt.getBlockPos().getZ());
	        	}
	        	else if(player.getHeldItem(hand.OFF_HAND).getItem().equals(Items_Magia_Varitas.VARITA_DE_RAYOS_PRINCIPIANTE)) 
	        	{
