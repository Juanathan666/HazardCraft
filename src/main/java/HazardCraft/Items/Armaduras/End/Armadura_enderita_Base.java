...
public class Armadura_enderita_Base extends ItemArmor implements Modelos {
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer player, EnumHand handIn) 
	{
		...
		if(casco.isEmpty() && (player.getHeldItemMainhand().getItem() == Registrar_Armaduras_End.casco_de_enderita || player.getHeldItemOffhand().getItem() == Registrar_Armaduras_End.casco_de_enderita)) {
              sonido(worldIn, player);
		}
		if(pechera.isEmpty() && (player.getHeldItemMainhand().getItem() == Registrar_Armaduras_End.pechera_de_enderita || player.getHeldItemOffhand().getItem() == Registrar_Armaduras_End.pechera_de_enderita)) {
			  sonido(worldIn, player);
		}
		if(grebas.isEmpty() && (player.getHeldItemMainhand().getItem() == Registrar_Armaduras_End.grebas_de_enderita || player.getHeldItemOffhand().getItem() == Registrar_Armaduras_End.grebas_de_enderita)) {
			  sonido(worldIn, player);
		}
		if(botas.isEmpty() && (player.getHeldItemMainhand().getItem() == Registrar_Armaduras_End.botas_de_enderita || player.getHeldItemOffhand().getItem() == Registrar_Armaduras_End.botas_de_enderita)) {
			  sonido(worldIn, player);
		}	
